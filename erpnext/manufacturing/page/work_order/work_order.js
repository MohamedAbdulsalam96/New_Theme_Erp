...
frappe.WOForm = Class.extend({
	...
	render_wo_form: function(){
		...
		$("<button class='btn btn-small btn-primary' style='margin-bottom:2%; margin-left:35%'><i class='icon-save'></i> Save </button>")
	...
	create_child_row: function(key, dic){
		...
		if(key == 'Style Transactions'){
			...
			$('<button  class="remove"><i class="icon-trash grid-delete-row"></i></button>').appendTo($("<td>")).appendTo(row)
				.click(function(){
					$(this).parent().remove()
			});
		}
		...
		frappe.call({
			...
			callback: function(r){
				if(r.message){
					...
					frappe.route_options.trial_no = r.message[1]
					window.history.back()
				}
			}
		})
